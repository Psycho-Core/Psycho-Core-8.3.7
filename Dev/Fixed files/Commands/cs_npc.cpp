...
public:
    ...
    */
    ...
    static bool HandleNpcAddWeaponCommand(ChatHandler* /*handler*/, char const* /*args*/)
    {
        ...
        uint64 guid = handler->GetSession()->GetPlayer()->GetSelectedUnit() ? handler->GetSession()->GetPlayer()->GetSelectedUnit()->GetGUID().GetRawValue() : 0;
        if (guid == 0)
        {
        ...
        if (tmpItem)
        {
            ...
            {
                case 1:
                    creature->SetVirtualItem(0, ItemID);
                    added = true;
                    break;
                case 2:
                    creature->SetVirtualItem(1, ItemID);
                    added = true;
                    break;
                case 3:
                    creature->SetVirtualItem(2, ItemID);
                    added = true;
                    break;
                default:
