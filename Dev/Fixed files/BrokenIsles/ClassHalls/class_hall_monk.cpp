  ...
  /*  void OnCheckPhase(Player* player)
    {
        ...
        {
            if (player->GetQuestStatus(QUEST_THE_WANDERERS_COMPANION) == QUEST_STATUS_REWARDED)
                PhasingHandler::AddPhase(player, 6789);
            if (player->GetQuestStatus(QUEST_JOURNEY_TO_THE_EAST) == QUEST_STATUS_REWARDED)
                PhasingHandler::RemovePhase(player, 6789);
        }
        if (player->GetAreaId() == 5972)
        {
            if (player->GetQuestStatus(QUEST_THE_RIDDLE_OF_PURITY) == QUEST_STATUS_INCOMPLETE)
                PhasingHandler::AddPhase(player, 6789);
        }*/
     /*   if (player->GetAreaId() == 5982)
        {
            if (player->GetQuestStatus(QUEST_THE_RIDDLE_OF_THE_BARREL) == QUEST_STATUS_INCOMPLETE)
                PhasingHandler::AddPhase(player, 6826);
            if (player->GetQuestStatus(QUEST_THE_RIDDLE_OF_THE_LAND) == QUEST_STATUS_INCOMPLETE)
                PhasingHandler::AddPhase(player, 6826);*/
    //    }
     ...
     //   {
          /*  if (player->CheckQuestStatus(QUEST_JOURNEY_TO_THE_EAST, CHECK_QUEST_COMPLETE_AND_REWARDED))
            ...
            if (player->GetQuestStatus(QUEST_THE_TRIAL_AT_THE_TEMPLE) == QUEST_STATUS_REWARDED)
