    ...
    /*void OnCheckPhase(Player* player) 
    {
        if (player->getClass() == CLASS_DRUID || player->getLevel() >= 98)
        {
            if (player->GetAreaId() == 2363)
            {
                ...
                if (player->GetQuestStatus(QUEST_TO_THE_DREAMGROVE) == QUEST_STATUS_REWARDED)
                    PhasingHandler::RemovePhase(player, 1526);
            }
            if (player->GetAreaId() == 7846 || player->GetZoneId() == 7846)
            {
                if (player->GetQuestStatus(QUEST_IN_DEEP_SLUMBER) == QUEST_STATUS_REWARDED)
                {
                    PhasingHandler::AddPhase(player, PHASE_IN_DEEP_SLUMBER);
                    PhasingHandler::AddPhase(player, 6356);
                }
                if (player->GetQuestStatus(QUEST_RECONVENE) == QUEST_STATUS_REWARDED)
                {
                ...
                if (player->GetQuestStatus(QUEST_CLEANSING_THE_MOTHER_TREE) == QUEST_STATUS_REWARDED)
