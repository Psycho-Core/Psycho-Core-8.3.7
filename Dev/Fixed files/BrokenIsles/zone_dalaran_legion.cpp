    ...
    { 
        if (player->GetMapId() == 1220)
        {
            if (player->GetAreaId() == 7505 && player->getClass() == CLASS_PALADIN)
            {
                ...
                if (player->GetQuestStatus(42120) == QUEST_STATUS_REWARDED && player->GetQuestStatus(42017) == QUEST_STATUS_REWARDED && player->GetQuestStatus(38376) == QUEST_STATUS_REWARDED)
            ...
            if (player->getClass() == CLASS_DEMON_HUNTER)
            {
                if ((player->GetAreaId() == 7505) || (player->GetAreaId() == 7592))
                {
                    ...
                    if (player->GetQuestStatus(41119) == QUEST_STATUS_REWARDED || player->GetQuestStatus(39247) == QUEST_STATUS_REWARDED || player->GetQuestStatus(40249) == QUEST_STATUS_REWARDED || player->GetQuestStatus(41863) == QUEST_STATUS_REWARDED)
                        PhasingHandler::AddPhase(player, PHASE_CLASS_DEMON_HUNTER_READY_TO);
                    // Eternal Vigil
                    if (player->GetQuestStatus(42869) == QUEST_STATUS_REWARDED && !player->HasQuest(41119) && !player->HasQuest(39247) && !player->HasQuest(40249) && !player->HasQuest(41863))
                ...
                if (player->GetAreaId() == 8284)
                {
                    ...
                    if (player->GetQuestStatus(42869) == QUEST_STATUS_REWARDED)
                        PhasingHandler::AddPhase(player, PHASE_CLASS_DEMON_HUNTER_READY_TO);

                    if (player->GetQuestStatus(42872) == QUEST_STATUS_INCOMPLETE)
                        PhasingHandler::AddPhase(player, 5023);
                    if (player->GetQuestStatus(42872) == QUEST_STATUS_REWARDED)
                    {
            ...
            if (player->getClass() == CLASS_PRIEST)
            {
                ...
                {
                    if (player->GetQuestStatus(41957) == QUEST_STATUS_REWARDED)
                        PhasingHandler::AddPhase(player, PHASE_CLASS_PRIEST);
                    if (player->GetQuestStatus(41967) == QUEST_STATUS_REWARDED)
                        PhasingHandler::RemovePhase(player, PHASE_CLASS_PRIEST);
                }
                if (player->GetAreaId() == 7505)
                {
                    if (player->GetQuestStatus(41967) == QUEST_STATUS_COMPLETE)
                        PhasingHandler::AddPhase(player, PHASE_CLASS_PRIEST);
                    if (player->GetQuestStatus(41967) == QUEST_STATUS_REWARDED)
                        PhasingHandler::RemovePhase(player, PHASE_CLASS_PRIEST);
                }
                ...
                {
                    if (player->GetQuestStatus(41967) == QUEST_STATUS_INCOMPLETE)
                    ...
                    if (player->GetQuestStatus(41993) == QUEST_STATUS_INCOMPLETE)
                    {
                        PhasingHandler::RemovePhase(player, 6265);
                        PhasingHandler::AddPhase(player, 6310);
                    }
                    if (player->GetQuestStatus(41993) == QUEST_STATUS_REWARDED)
                    {
                        PhasingHandler::RemovePhase(player, 6265);
                    ...
                    if (player->GetQuestStatus(42074) == QUEST_STATUS_REWARDED)
                    {
                        PhasingHandler::RemovePhase(player, 6265);
                        PhasingHandler::RemovePhase(player, 6310);
            ...
            {
                if ((player->GetAreaId() == 7505) || (player->GetAreaId() == 7592))
                    if (player->GetQuestStatus(40714) == QUEST_STATUS_REWARDED)
        ...
        if (player->GetAreaId() == 7505)
        {
            if (player->GetQuestStatus(46734) != QUEST_STATUS_REWARDED)
                PhasingHandler::AddPhase(player, PHASE_QUEST_ASSAULT_ON_BROKEN_SHORE);
            else if (player->GetQuestStatus(46734) == QUEST_STATUS_REWARDED)
                PhasingHandler::RemovePhase(player, PHASE_QUEST_ASSAULT_ON_BROKEN_SHORE);
            if (player->GetQuestStatus(39731) == QUEST_STATUS_REWARDED || player->GetQuestStatus(39861) == QUEST_STATUS_INCOMPLETE)
                PhasingHandler::AddPhase(player, PHASE_QUEST_VALSHARAH);
            if (player->GetQuestStatus(39861) == QUEST_STATUS_REWARDED)
                PhasingHandler::RemovePhase(player, PHASE_QUEST_VALSHARAH);

            if (player->GetQuestStatus(39718) == QUEST_STATUS_REWARDED || player->GetQuestStatus(41220) == QUEST_STATUS_INCOMPLETE)
                PhasingHandler::AddPhase(player, PHASE_QUEST_AZUNA);
            if (player->GetQuestStatus(41220) == QUEST_STATUS_REWARDED)
        ...
        if (player->GetAreaId() == 7502)
        {
         ...
         /*   if (player->CheckQuestStatus(39864, CHECK_QUEST_TAKEN_AND_COMPLETE_AND_REWARDED) || player->CheckQuestStatus(44701, CHECK_QUEST_TAKEN_AND_COMPLETE_AND_REWARDED))
            ...
            if (player->GetQuestStatus(39801) == QUEST_STATUS_REWARDED)
                PhasingHandler::RemovePhase(player, PHASE_STORMHEIM_HORDE);
            //alliance
            if (player->GetQuestStatus(39735) == QUEST_STATUS_REWARDED || player->GetQuestStatus(44700) == QUEST_STATUS_REWARDED)
                PhasingHandler::AddPhase(player, PHASE_STORMHEIM_ALLIANCE);
            if (player->GetQuestStatus(38052) == QUEST_STATUS_REWARDED)
