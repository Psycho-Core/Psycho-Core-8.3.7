   ...
   /* void SpellFinishCast(const SpellInfo* spell) override
    {
        ...
        case SPELL_ANNIHILATION:
            ...
            me->GetScheduler().Schedule(Milliseconds(500), [this](TaskContext)
            {
                ...
                me->GetScheduler().Schedule(Milliseconds(2000), [this](TaskContext)
                {
                    DoCast(me, SPELL_SEAR, true);
                });
                me->SetDisplayId(74497);

                me->GetScheduler().Schedule(Milliseconds(5000), [this](TaskContext)
                {
                    ...
                    for (auto pair : perephaseEvent)
                        ...
                        {
                            cre->SetWalk(false);
                            cre->GetMotionMaster()->MoveSmoothPath(0, pair.second.second.data(), pair.second.second.size(), false, true);
                            cre->SetCanFly(true);
                        }
                    me->GetScheduler().Schedule(Milliseconds(3000), [this](TaskContext){ me->CastSpell(me, SPELL_CONVERS_EVENT_1, false); });
                });

                me->GetScheduler().Schedule(Milliseconds(7000), [this](TaskContext)
                {
                    instance->instance->ApplyOnEveryPlayer([](Player* player) -> void
                    {
